...
dim(ungrazed)
dim(grazed)
colSums(ungrazed[,2:13]); colSums(grazed[,2:13])
rowSums(ungrazed[,2:13]); rowSums(grazed[,2:13])

wUG1<-ungrazed[,2:13]    # 70x12
wG1<-(grazed[,2:13])     # 120x12

# I trimm the grazed dataset to delete trees so that I get N=70.
# Trees with 0 records are included.
# 
wG1<- wG1 %>% 
           dplyr::filter(rowSums(wG1^2)!=0) 
dim(wG1)    # 106x12

rG1<- sample_n(wG1, 70, replace= F)   # Random sample from wG1 with N=70 trees (without replacement).
dim(rG1)
dim(wUG1)
...
# same dimension.
grG1<-network(rG1, vertex.attr=NULL, vertex.attrnames=NULL, directed=F,hyper=FALSE, matrix.type="adjacency", multiple=FALSE, bipartite = 70)
gwUG1<-network(wUG1, vertex.attr=NULL, vertex.attrnames=NULL, directed=F,hyper=FALSE, matrix.type="adjacency", multiple=FALSE, bipartite = 70)

#Perform qap tests of graph correlation
prosop<- array(dim=c(2,70,12))
prosop[1,,]<- rG1
prosop[2,,]<- wUG1

g08<-array(dim=c(2,15,58))
g08[1,,]<-tei08pre.qt
g08[2,,]<-tei08pos.qt[1:15,]
g09<-array(dim=c(2,15,68))
g09[1,,]<-tei09pre.qt
g09[2,,]<-tei09pos.qt[1:15,]

#Perform qap tests of graph correlation
q.08<-qaptest(g08,gcor,g1=1,g2=2)
q.09<-qaptest(g09,gcor,g1=1,g2=2)
...
# hdist
q.Hd08<-qaptest(g08,hdist,g1=1,g2=2)
q.Hd09<-qaptest(g09,hdist,g1=1,g2=2)
summary(q.Hd08)
summary(q.Hd09)
plot(q.12,xlim=c(7000,12000))
...
structdist(g08,method="anneal", prob.init=0.9, prob.decay=0.85, 
    freeze.time=50, full.neighborhood=TRUE)
structdist(g09,method="anneal", prob.init=0.9, prob.decay=0.85, 
           freeze.time=50, full.neighborhood=TRUE)
