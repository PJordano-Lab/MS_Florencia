...
```
```{r cca CCorA, echo= TRUE, message= TRUE, warning= TRUE}
library(vegan)
ccaG<-CCorA(plantG, fruG, stand.Y=T, stand.X=T, permutations = 999)
ccaG

```
```{r cca cancor, echo= TRUE, message= TRUE, warning= TRUE}
library(candisc)
...
library(candisc)
cch <- cancor(fruGh, plantGh)
cch
#Wilks(cc)

ccRh <- cancor(fruUGh, plantUGh)
ccRh
#Wilks(ccR)
...
```{r biplot mesohabitat Grazed sites, echo= TRUE, message= TRUE, warning= TRUE, fig.width= 14, size= 0.01}
...
```{r biplot mesohabitat Ungrazed sites, echo= TRUE, message= TRUE, warning= TRUE, fig.width= 14, size= 0.01}
